...
/- OS2 ℝᵈ with d = 4, where μ is the Lebesgue measure on R^4 as mu. -/
def STDimension := 4
...
/-! abbrev field space' is an alias for L^2(R^4,mu, R) -/
abbrev FieldSpace' := Lp (p := 2) (μ := μ) ℝ

/-apply standard Borel sigma algebra in the space-/
instance : MeasurableSpace FieldSpace' := borel _
instance : BorelSpace FieldSpace' := ⟨rfl⟩

/-- Orthogonal linear isometries of ℝ⁴ (the group O(4)). Objets are linear iso of R^4 (ortho mat)-/
...
/- Action of a motion on a point, then rotation and translation-/
...
/- inv builds the group inverse of an isometry -/
noncomputable def inv (g : O4) : O4 :=
  ((g.toLinearIsometryEquiv rfl).symm).toLinearIsometry

  /- give simplification rules ofr composition and inverse-/
...
/- Extensionality principle: two motions are equal if their rotation and translation parts coincide-/
@[ext] lemma E.ext {g h : E} (hR : g.R = h.R) (ht : g.t = h.t) : g = h := by
  cases g; cases h; cases hR; cases ht; rfl

/-! ## Group structure on `E` ------------------------------------------------ -/

/- Provide mul, one, in,div instances for E. Mulip is a demi direct product formula
   identity is (1,0); inverse uses the orthogonal inverse; division is defined via inverse. -/
...
/- confirms associativity and rotation of composition -/
...
/- show the action respects group multiplication and inversion
...
/-! Defne aliases of L^2 field spaces and give Borel instances-/
...
/- declare variables, a point x, a field phi and probability measures dmu and dmu'-/
...
/-- measurePreserving_act proves that every Euclidean motion preserves
Lebesgue measure by composing a rotation (isometry) with a translation (translation invariance).
...
/- gives motion inverse in Euclidean group-/
@[simp] def invE (g : E) : E :=
  ⟨LinearIsometry.inv g.R, -(LinearIsometry.inv g.R) g.t⟩

/-Lemmas contDiff_act_inv, measurable_act_inv, mp_act_inv establish smoothness,
measurability, and measure-preservation for the inverse action.-/
...
/- Big helper block building technical estimates: polynomial growth, temperate growth, derivatives of the inverse action,
all needed so SchwartzMap.compCLM will accept the map as temperate-/
...
-- Helper: A continuous linear map is differentiable at every point
private lemma clm_differentiableAt (L : RSpaceTime →L[ℝ] RSpaceTime) (x : RSpaceTime) :
    DifferentiableAt ℝ (⇑L) x :=
  L.differentiableAt

-- Helper: fderiv of linear map plus constant equals fderiv of linear map
private lemma fderiv_linear_add_const (L : RSpaceTime →L[ℝ] RSpaceTime) (c : RSpaceTime) (x : RSpaceTime) :
    fderiv ℝ (fun y => L y + c) x = fderiv ℝ L x := by
  -- Attempt 10: Try the simplest form - just apply the theorem
  -- The theorem: fderiv ℝ (fun x => f x + c) = fderiv ℝ f
  -- Our goal: fderiv ℝ (fun y => L y + c) x = fderiv ℝ L x
  -- These match exactly if L is seen as a function
  apply fderiv_add_const

-- Helper: fderiv of act (invE g) equals a linear map, uses function to supress warning for unused x variable
set_option linter.unusedVariables false in
private def fderiv_act_invE_eq_linear (g : E) :
  (fun x => fderiv ℝ (act (invE g)) x) = fun x => (LinearIsometry.inv g.R).toContinuousLinearMap := by
  ext x v i
  -- ...rest of your proof...
  -- Compute fderiv of the affine map x ↦ (LinearIsometry.inv g.R) x + (invE g).t
  -- This should equal (LinearIsometry.inv g.R).toContinuousLinearMap applied to v

  -- Step 9: More explicit about coercions
  let L := (LinearIsometry.inv g.R).toContinuousLinearMap
  -- Show the computation step by step
  calc (fderiv ℝ (act (invE g)) x v) i
      = (fderiv ℝ (fun y => L y + (invE g).t) x v) i := by
          -- act (invE g) equals this affine function
          simp only [invE]
          rfl
      _ = ((fderiv ℝ (fun y => L y + (invE g).t) x) v) i := rfl
      _ = ((fderiv ℝ L x) v) i := by
          -- fderiv of f + const is fderiv of f
          rw [fderiv_linear_add_const]
      _ = (L v) i := by
          -- fderiv of continuous linear map is itself
...
private def fderiv_has_temperate_growth (g : E) :
  ...
  -- The fderiv of an affine map is constant, so we can use HasTemperateGrowth.const
  rw [fderiv_act_invE_eq_linear g]
...
/-TestFunction.push
Defines how a Euclidean motion pushes a Schwartz function: pre-compose with the inverse action, using the proofs just assembled.-/
...
/- FieldSpace.pull
Defines the contravariant pullback of an L² field: compose with inverse motion and use Lp.compMeasurePreserving
to stay in L² since the action preserves measure.-/
set_option linter.unusedVariables false in
noncomputable def FieldSpace.pull (g : E) (φ : FieldSpace) : FieldSpace :=
  Lp.compMeasurePreserving (p := 2) (act (invE g)) (mp_act_inv g) φ

/-EuclideanInvariant predicate says a field law is invariant if its
pushforward under every motion equals itself.-/
...
/-axiom OS2 … records the Osterwalder–Schrader axiom:
the chosen probability measure is Euclidean invariant.-/
axiom OS2 (dμ : ProbabilityMeasure FieldSpace) :
  EuclideanInvariant dμ

/- checking the axiom-/

/- It eats a Euclidean motion g, an L² field φ, and a point x. It spits out a proposition
(boolean statement) that we will test.-/
def check_implementation (g : E) (φ : FieldSpace) (x : RSpaceTime) : Prop :=
/- let pulled: computes the pulled-back field: pre-compose φ with the inverse of g,
remaining safely in L² because g is measure-preserving.-/
  let pulled := FieldSpace.pull g φ
  let original := φ
  /-Unpack both L² functions into honest measurable functions and assert equality at one spatial point:
value of the pulled field at x equals value of the original field at the point that moves backwards by g.
If the pull definition is correct, this tautology holds. -/
  (pulled : RSpaceTime →ₘ[μ] ℝ) x = (original : RSpaceTime →ₘ[μ] ℝ) (act (invE g) x)

set_option linter.unusedVariables false

/- Define a concrete proposition that will be true if our machinery is coherent.-/
def verification_example : Prop :=

  let rotation : O4 := (1 : O4)  -- trivial rotation = I for simplicity
  let translation : RSpaceTime := fun i => 1 -- Translation by (1,1,1,1), shifts every coord by 1
  /- Package the rotation and translation into a single Euclidean motion g.-/
  let g : E := ⟨rotation, translation⟩

  -- state that For any field φ and point x, check that pull g φ works correctly
  ∀ (φ : FieldSpace) (x : RSpaceTime), check_implementation g φ x

#check OS2 -- Show that our axiom has the correct type

/- The output of th check is: QFT.OS2 (dμ : ProbabilityMeasure ↥FieldSpace) : EuclideanInvariant dμ-/
